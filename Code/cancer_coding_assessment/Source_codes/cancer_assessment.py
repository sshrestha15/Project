...
plt.scatter(counts_mutation_response ['Responder'] + counts_mutation_response ['Non-Responder'], counts_mutation_response ['p_value'])
plt.xlabel('Number of Mutated Patients')
plt.ylabel("P-Value")
plt.title("Number of Mutations for Treatment from Patients Response")
plt.grid(True)
plt.axhline(y=0.05, color='g', linestyle='dashed', label="P-Value threshold (p-value < 0.05)")
plt.legend()
plt.show()
...
plt.title(f"Mutation Rate in {most_significant_genes} Mutant vs. Wild-type Samples")
plt.show()
